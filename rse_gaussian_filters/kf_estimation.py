...
from .sensor_utils import odom_to_pose2D, get_normalized_pose2D
...
class KalmanFilterNode(Node):
    def __init__(self):
        ...
        self.odom_raw_subscription = self.create_subscription(
            Odometry,
            'odom_raw',         # For controls
            self.odom_raw_callback,
            10)

        # Initialize the Kalman Filter
        self.kf = KalmanFilter(np.zeros(3), np.eye(3), proc_noise_std = [2.0, 2.0, 2.0], obs_noise_std = [0.02, 0.02, 0.01])
    ...
    def odom_raw_callback(self, msg):
        self.u = np.asarray([msg.twist.twist.linear.x, msg.twist.twist.angular.z]) # Get the control inputs
